...
(defn disable-drag []
  (swap! app-state update :positions #(mapv disable-item %)))

(defn enable-top-drag []
  (->> (group-by :stack (:positions @app-state))
       (run! (fn [[_ v]]
               (swap! app-state update-in [:positions (:index (first v))] enable-item)))))
...
(defmethod event-handler ::mouse-pressed [& {:keys [index]}]
  (swap! app-state update :positions #(mapv to-background %))
  (swap! app-state update-in [:positions index] assoc :view-order -1))
...
(defmethod event-handler ::mouse-dragged [& {:keys [index width height] :as e}]
  (let [fx-event ^MouseEvent (:fx/event e)
    ...
    (swap! app-state update-in [:positions index] assoc
...
(defmethod event-handler ::mouse-released [& {:keys [index stack] :as event}]
  (let [fx-event ^MouseEvent (:fx/event event)
    ...
    (swap! app-state update :moves (if legal-move? inc identity))
    (swap! app-state update-in [:positions index] assoc
...
(defn controls [state]
  (let [discs (:discs state)]
    [{:fx/type :label
     ...
     {:fx/type          :spinner
      :max-width        75
      :on-value-changed #(reset! app-state (make-state %))
     ...
     {:fx/type   :button
      :on-action (fn [_] (reset! app-state (make-state discs)))
      :text      "Reset"}
     {:fx/type :label
